...
export const SelectColor: React.FC<SelectColorProps> = ({
    ...
    const handleFileChange = useCallback((value: File) => {
        setFile(value);
        addImageToState({ ...item, imageUrl: value })
    }, []);
    ...
    return (
        <div className="grid grid-cols-1 overflow-y-auto border-b-2
         border-slate-200 items-center p-2
        ">
            <div className="flex flex-row gap-2 items-center h-[60px]">
                <input type="checkbox" id={item.color} checked={isSelected} onChange={handleCheck}
            ...
            <>
                ...
                {file && (
                    <div className="flex flex-row gap-2 text-small col-span-2 items-center justify-between">
                        <p>{file?.name}</p>
